...
DIRECTIONS = [Coord(i, j, k, l) for i in range(-1, 2) for j in range(-1, 2) for k in range(-1, 2) for l in range(-1, 2) if i != 0 or j != 0 or k != 0 or l != 0]
...
for i, l in enumerate(LINES):
    ...
    for j, c in enumerate(l):
        STATE[Coord(len(LINES) - i - 1, j, 0, 0)] = c
...
print(limit_x, limit_y, limit_z, limit_w)
    ...
    for i in range(limit_x[0], limit_x[1] + 1):
        for j in range(limit_y[0], limit_y[1] + 1):
            for k in range(limit_z[0], limit_z[1] + 1):
                for l in range(limit_z[0], limit_z[1] + 1):
                    c = Coord(i, j, k, l)
                    s = cur_state[c]
                    ...
                    if (s == "#" and (active_neighbors == 2 or active_neighbors == 3) or
                        s == "." and active_neighbors == 3):
                            ...
                            limit_w = update_limit(limit_w, c.w)
